...
# add share all layers (no specific layer)
...
for modality in range(0,3):
    ...
    plt.plot([1, 2, 3, 4, 5, 6], [x[modality] for x in all_res], 's-', label='$\it{S_{dim}}$=1024', color='r', markersize=10)
    plt.fill_between([1, 2, 3, 4, 5, 6], [x[modality] - x[modality + 3] for x in all_res], [x[modality] + x[modality + 3] for x in all_res], alpha=0.3, color='r')
    ...
    'testfm69-vgg-unified_trans-lr5e-4-bs-144-balbal-mix0.5-ls1-ld0.5-lst1-ldaTrue-e1024-se16-h4-di0-ds6-asl30-vsl30-normTrue-feat2-a0.5-noiseTrue']
    ...
    exp_num = len(exp_list)
    plt.plot([1, 2, 3, 4, 5, 6], [x[modality] for x in all_res], 'o-', label='$\it{S_{dim}}$=16', color='b', markersize=10)
    plt.fill_between([1, 2, 3, 4, 5, 6], [x[modality] - x[modality + 3] for x in all_res], [x[modality] + x[modality + 3] for x in all_res], alpha=0.3, color='b')

    plt.yticks(fontsize=12)
    plt.xticks([1,2,3,4,5,6], [1,2,3,4,5,'\t6 (all)'], fontsize=12)
...
# figure.supylabel()
plt.savefig('performance_slayer_rev.pdf', bbox_inches='tight', dpi=300)
    #plt.close()