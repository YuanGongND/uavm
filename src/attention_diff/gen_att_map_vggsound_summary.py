...
def gen_att_map(audio_model, val_loader, exp_name):
    ...
    with torch.no_grad():
        for i, (audio_input, video_input, labels) in enumerate(val_loader):
            ...
            # compute output
            audio_output, audio_att_map = audio_model(audio_input, 'a')
            for o_layer in range(len(audio_att_map)):
                A_a_feat[o_layer].append(audio_att_map[o_layer])
            video_output, video_att_map = audio_model(video_input, 'v')
            for o_layer in range(len(video_att_map)):
                A_v_feat[o_layer].append(video_att_map[o_layer])
    ...
    for o_layer in range(len(video_att_map)):
        ...
        att_diff.append(mae(A_a_feat[o_layer], A_v_feat[o_layer]))
        # save attention maps in corresponding folder
...
def get_eval(model, data, audio_conf, label_csv, num_class, model_type='cnn', batch_size=48, exp_name=None):
    ...
    if 'unified_trans' in model_type:
        ...
        audio_model = models.UnifiedTransVisual(embed_dim=embed_dim, num_heads=4, depth_i=depth_i, depth_s=depth_s, label_dim=num_class, a_seq_len=30, v_seq_len=30, a_input_dim=1024, v_input_dim=1024, s_embed_dim=s_embed_dim)
    ...
    # the difference between audio and visual attention maps
