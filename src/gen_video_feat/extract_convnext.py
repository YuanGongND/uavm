# code modified from https://github.com/antoine77340/video_feature_extractor
...
import argparse
from model_convnext import get_model
...
parser.add_argument('--model_size', type=int, default=2,
                            help='convnext model size, 2 is the base model used in the UAVM paper')
...
parser.add_argument('--l2_normalize', type=int, default=0,
...
dataset = VideoLoader(
    args.csv,
    framerate=3 if args.type == '2d' else 24,
    ...
    for k, data in enumerate(loader):
        ...
        if len(data['video'].shape) > 3:
            ...
            if len(video.shape) == 4:
                ...
                n_chunk = len(video)
                features = th.cuda.FloatTensor(n_chunk, hid_dim[args.model_size]).fill_(0)
