...
#SBATCH -n 1
#SBATCH --mem=120000
#SBATCH --job-name="as_uavm"
#SBATCH --output=../log/%j_as_uavm.txt
...
exp_dir=./exp/test01-as_${set}-${model}-lr${lr}-bs-${batch_size}-bal${bal}-mix${mixup}-ls${lrscheduler_start}-ld${lrscheduler_decay}-lst${lrscheduler_step}-lda${lr_adapt}-e${embed_dim}-se${s_embed_dim}-h${num_heads}-di${depth_i}-ds${depth_s}-asl${a_seq_len}-vsl${v_seq_len}-norm${feat_norm}-a${a_tr_prob}-noise${noise}-r${r}
mkdir -p $exp_dir
